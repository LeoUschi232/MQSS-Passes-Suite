...
A2C_CONV4::A2C_CONV4(unsigned int max_qubits,
  ...
  // decreasing.
  unsigned int L2 = static_cast<unsigned>(4.0 / 5.0 * IRS);
  L2 = std::max(5u, L2);
  unsigned int L3 = static_cast<unsigned>(3.0 / 5.0 * IRS);
  L3 = std::max(4u, L3);
  unsigned int L4 = static_cast<unsigned>(2.0 / 5.0 * IRS);
  L4 = std::max(3u, L4);
  unsigned int L5 = static_cast<unsigned>(1.0 / 5.0 * IRS);
  L5 = std::max(2u, L5);
